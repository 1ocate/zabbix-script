...
def getTriggerProtoTypeList(session,template_id_to_name):
    priority = [
        "Not classified",
        "Information",
        "Warning",
    ...
    #triggers': [{'triggerid': '17117', 'expression': '{20124}<1.597 or {20124}>2.019', 'description': 'BB +1.8V SM Critical [{ITEM.VALUE}]', 'url': '', 'status': '0', 'value': '0', 'priority': '5', 'lastchange': '0', 'comments': '', 'error': '', 'templateid': '0', 'type': '0', 'state': '0', 'flags': '0', 'recovery_mode': '0', 'recovery_expression': '', 'correlation_mode': '0', 'correlation_tag': '', 'manual_close': '0', 'opdata': ''
    #item_datas = session.item.get({ "selectTriggers": ["expression","description","status","recovery_expression","priority" "selectApplications","opdata"], "expandExpression": "y", "selectApplications": "true", "output": ["hostid","name","key_","delay","history"], "templateids":templateids })
    trigger_datas = session.triggerprototype.get({ "selectTriggers": ["triggerid"],  "expandExpression": "y", "expandDescription": "y", "output": ["expression","description","status","recovery_expression","priority", "opdata"], "templateids":templateids })
    result = {}
...
def getItemProtoTypeList(session,template_id_to_name):
    templateids = list(template_id_to_name.keys())
    itemprototype_datas = session.itemprototype.get({  "selectTriggers": ["triggerid"], "selectApplications": "true", "output": ["hostid", "name", "key_", "delay", "history", "status"], "templateids":templateids })
    newItem_list= []
    if len(itemprototype_datas) != 0:
        for item in itemprototype_datas:
            status = ''
            if item.get('status') == '0':
                status = '활성'
            elif item.get('status') == '1':
...
triggerprototype_id_to_data = getTriggerProtoTypeList(session,template_id_to_name)
item_list = getItemList(session,template_id_to_name)
itemprototype_list = getItemProtoTypeList(session,template_id_to_name)
...
for item in itemprototype_list:
    if (len(item[9]) == 0):
       item[9] = ""
    else:
       item[9] = "\n!!!!!!!!!".join([triggerprototype_id_to_data[sub_dict['triggerid']] for sub_dict in item[9]])
    print('!'.join(map(str, item)))
